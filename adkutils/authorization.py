...
def list(project_id: str, location: str, format_raw: bool = False):
    ...
    print_list(helper.get("authorizations"), format_raw)

@app.command()
def create(project_id: str, location: str, auth_id: str, client_id: str, 
           client_secret: Annotated[str, typer.Option(prompt=True, confirmation_prompt=True, hide_input=True)], 
    ...
    auth_uri = generate_auth_uri(base_auth_uri=base_auth_uri,
        client_id=client_id,
        scopes=scopes)
    ...
    try:
        response = helper.post(f"authorizations?authorizationId={auth_id}", data=payload)
