...
@app.command()
def create_or_update(project_id: str, location: str, gemini_app_id: str,
             display_name: str, description: str, tool_description: str,
    ...
    payload = {
        ...
        "adk_agent_definition": {
            ...
            "authorizations": [f"projects/{project_num}/locations/{location}/authorizations/{auth_id}" for auth_id in auth_ids]
        }
    }
    if icon_uri:
    ...
    try:
        if existing_agent_id:
            response = helper.patch(f"collections/default_collection/engines/{gemini_app_id}/assistants/default_assistant/agents/{existing_agent_id}", 
                     data=payload)
        else:
            response = helper.post(f"collections/default_collection/engines/{gemini_app_id}/assistants/default_assistant/agents", 
                     data=payload)
...
def delete(project_id: str, location: str, gemini_app_id: str, agent_id: str):
    helper = DiscoveryEngineRequestHelper(project_id,location)
    try:
        response = helper.delete(f"collections/default_collection/engines/{gemini_app_id}/assistants/default_assistant/agents/{agent_id}")
...
def print_list(data):
    ...
    for agent in data.get('agents', []):
        ...
        reasoning_engine_name = provisioned_engine.get('reasoningEngine', 'N.A').split('/')[-1]
        authorizations = adk_definition.get('authorizations', [])
        auth_ids = ", ".join([auth.split('/')[-1] for auth in authorizations]) if authorizations else "N.A"
...
def list(project_id: str, location: str, app_id: str):
    helper = DiscoveryEngineRequestHelper(project_id,location)
    print_list(helper.get(f"collections/default_collection/engines/{app_id}/assistants/default_assistant/agents"))
